...
- [ ] SDK-44 Exchange data through an exposed TCP port where ingress is configured. PARTIAL: production TCP ingress lacks public DNS, TLS, and listener routing. Local TCP passed. Terraform now declares a DNS-only `*.tcp.lazycloud.dev` CNAME with a required TCP load balancer target. Formatting and provider validation passed; no live plan or apply ran. The listener and trusted certificate remain unfinished. Removed the unused ExternalDNS/cert-manager draft, its Helm dependency, and its application DNS token binding. No ingress change shipped in 0.0.97.
