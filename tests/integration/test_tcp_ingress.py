...
def test_tcp_route_resolver_uses_active_public_pod_hierarchy_and_revalidates_cache(
    ...
    cache_key = isolated_services.redis().key("tcp-ingress", "sni", sni)
    assert isolated_services.redis().ttl(cache_key) > 0
    ...
    assert isolated_services.redis().get(cache_key) is None


def test_tcp_route_resolver_rejects_private_non_tcp_and_unexposed_pods(
    isolated_services: ApiServices,
) -> None:
    ...
    for stub in (private, not_tcp, wrong_port):
        with pytest.raises(TcpIngressRouteNotFound):
            resolver.resolve(tcp_ingress_hostname(stub.id, 9090, "tcp.example.test"))


def test_tls_context_captures_sni_and_reloads_atomically_rotated_certificate(
    tmp_path: Path,
) -> None:
    ...
    tls = ReloadingTlsContext(certificate_file, key_file)
