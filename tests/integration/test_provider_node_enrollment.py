    ...
    try:
        with service_graph(
            ...
            request = ProviderNodeEnrollmentRequest(
                ...
                preflight=[
                    ComputePreflightCheck(name="docker", ok=True, severity=PreflightSeverity.Error)
                ],
            )
            with database.session() as session:
                session.execute(
...
def _request(
    ...
    *,
    ...
    return ProviderNodeEnrollmentRequest(
        ...
        preflight=[ComputePreflightCheck(name="docker", ok=True, severity=PreflightSeverity.Error)],
    )
