...
func (n *FSNode) Getxattr(ctx context.Context, name string, dest []byte) (uint32, syscall.Errno) {
	for _, attr := range n.clipNode.Xattrs {
		if attr.Name == name {
			if len(dest) == 0 {
				return uint32(len(attr.Value)), fs.OK
			}
			if len(dest) < len(attr.Value) {
				return uint32(len(attr.Value)), syscall.ERANGE
			}
			return uint32(copy(dest, attr.Value)), fs.OK
		}
	}
...
func (n *FSNode) Listxattr(ctx context.Context, dest []byte) (uint32, syscall.Errno) {
	...
	offset := 0
	for _, attr := range n.clipNode.Xattrs {
		offset += copy(dest[offset:], attr.Name)
		dest[offset] = 0
		offset++
	}
	return uint32(size), fs.OK
