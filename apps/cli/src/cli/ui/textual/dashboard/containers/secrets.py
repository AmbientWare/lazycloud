...
from cli.ui.textual.dashboard.containers.details.secret_details import (
    SecretDeleted,
    SecretUpdated,
)
from cli.ui.textual.messages import SecretSelected
...
class SecretsContainer(Container):
    ...
    async def _update_count(self, deployment_id: str) -> None:
        """Fetch and update the secret count."""
        try:
            secrets = await api.secrets.get_secrets(deployment_id)
            self.secret_count = (
    ...
    def watch_selected_secret_key(self, _old_value, new_value) -> None:
        """React when a secret is selected - post message for other components"""
        self.log(
