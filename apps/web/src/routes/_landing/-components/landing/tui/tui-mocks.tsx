        ...
        {metrics.map((metric) => (
          <div key={metric.label} className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              ...
              <span className="w-12 text-right text-lazycloud">
                {metric.cost}
              </span>
            </div>
          </div>
    ...
    <div className="font-mono text-xs">
      ...
      <div className="space-y-1.5 rounded border border-border/40 bg-muted/20 p-2">
        ...
        <div className="flex gap-2 text-[11px]">
          <span className="w-10 shrink-0 font-semibold text-green-500">
            INFO
          </span>
    ...
    <div className="font-mono text-xs">
      ...
      <div className="space-y-2">
        {actions.map((action, idx) => (
          ...
          >
            <div className="flex items-center gap-2">
              <span className="flex size-5 items-center justify-center rounded border border-border/60 bg-muted/40 text-[10px] font-semibold text-muted-foreground">
                {action.key}
              </span>
              <span className="text-foreground">{action.name}</span>
            </div>
            <span className="text-[10px] text-green-500">{action.status}</span>
          </div>
        ))}
      </div>
    </div>
