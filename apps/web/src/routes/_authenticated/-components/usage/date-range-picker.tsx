...
import { dateToUrlString } from '@/lib/format-date'
...
export function DateRangePicker({
  ...
  const findMatchingPreset = (): Preset | undefined => {
    ...
    const checkPreset = (preset: Preset): boolean => {
      ...
      return (
        presetFromNorm === currentFromNorm && presetToNorm === currentToNorm
      )
    }
    ...
    for (let i = allPresets.length - 1; i >= 0; i--) {
      ...
      if (
        preset &&
        preset.id !== 'this-month' &&
        preset.id !== 'billing-cycle' &&
        checkPreset(preset)
      ) {
        return preset
      }
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent className="w-[--radix-popover-trigger-width] p-0 sm:w-[280px]">
        <Command>
          <CommandList>
            <CommandGroup>
