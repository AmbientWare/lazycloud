...
export function AppLifecycleActions({
  ...
  const refresh = async () => {
    await Promise.all([
      queryClient.invalidateQueries({ queryKey: workspaceQueryKeys.apps.detail(workspaceId, app.id) }),
  ...
  const pause = useMutation({ ...pauseAppMutationOptions(workspaceId, app.id), onSuccess: refresh });
  const resume = useMutation({ ...resumeAppMutationOptions(workspaceId, app.id), onSuccess: refresh });
  ...
  const hasActions =
    app.actions.can_pause || app.actions.can_resume || app.actions.can_delete;
