...
export function podInstanceUptime(container: Container): string {
  if (!container.started_at) return "-";
  const started = Date.parse(container.started_at);
  const finished = container.finished_at ? Date.parse(container.finished_at) : Date.now();
  if (Number.isNaN(started) || Number.isNaN(finished) || finished < started) return "-";
  const elapsed = finished - started;
  const days = Math.floor(elapsed / 86_400_000);
  if (days === 0) return durationBetween(container.started_at, container.finished_at) ?? "-";
