...
export function deleteWorkloadMutationOptions(workspaceId: string, appId: string, name: string) {
  return {
    mutationFn: () =>
      apiRequest(
        withWorkspace(
          `/api/v1/apps/${encodeURIComponent(appId)}/workloads/${encodeURIComponent(name)}`,
          workspaceId,
        ),
        noContentSchema,
        { method: "DELETE" },
