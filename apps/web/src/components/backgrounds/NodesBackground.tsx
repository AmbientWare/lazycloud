...
function GraphNodes({ seed }: GraphNodesProps) {
  ...
  const edges = useMemo(() => generateEdges(initialNodes, seed), [initialNodes, seed]);
...
// Client-side only version of NodesBackground
export default dynamic(() => Promise.resolve(NodesBackground), {
  ssr: false,
});
