...
export function WorkspaceUsageCard({
  ...
  const { activeDeployments, allInactiveDeployments } = useMemo(() => {
    ...
    const inactive = deployments
      ...
      .sort((a, b) => {
        const nameCompare = (a.deployment_name || "").localeCompare(b.deployment_name || "");
        if (nameCompare !== 0) return nameCompare;
        // Secondary sort by deleted_at date
                      ...
                      <thead>
                        <tr className="border-b bg-muted">
                          ...
                          <th className="text-right p-3 font-semibold">
                            Cost<span className="text-muted-foreground font-normal text-xs ml-1">($)</span>
                          </th>
                        </tr>
                      </thead>
                      <tbody>
                        <tr className="border-b bg-card">
                          <td className="p-3"><span className="font-medium">CPU (core-hrs)</span></td>
                          <td className="p-3 text-right">{formatUsageValue(deploymentWithBreakdown.usage.cpu_core_hours)}</td>
                          <td className="p-3 text-right">
                            {deploymentWithBreakdown.usage.costs ? (
                              <span className="font-semibold">{deploymentWithBreakdown.usage.costs.cpu_cost.toFixed(2)}</span>
                            ) : (<span className="text-muted-foreground">-</span>)}
                          </td>
                        </tr>
                        <tr className="border-b bg-card">
                          <td className="p-3"><span className="font-medium">Memory (GB-hrs)</span></td>
                          <td className="p-3 text-right">{formatUsageValue(deploymentWithBreakdown.usage.memory_gb_hours)}</td>
                          <td className="p-3 text-right">
                            {deploymentWithBreakdown.usage.costs ? (
                              <span className="font-semibold">{deploymentWithBreakdown.usage.costs.memory_cost.toFixed(2)}</span>
                            ) : (<span className="text-muted-foreground">-</span>)}
                          </td>
                        </tr>
                        <tr className="border-b bg-card">
                          <td className="p-3"><span className="font-medium">Storage (GB-hrs)</span></td>
                          <td className="p-3 text-right">{formatUsageValue(deploymentWithBreakdown.usage.standard_gb_hours + deploymentWithBreakdown.usage.shared_gb_hours)}</td>
                          <td className="p-3 text-right">
                            {deploymentWithBreakdown.usage.costs ? (
                              <span className="font-semibold">{(deploymentWithBreakdown.usage.costs.standard_cost + deploymentWithBreakdown.usage.costs.shared_cost).toFixed(2)}</span>
                            ) : (<span className="text-muted-foreground">-</span>)}
                          </td>
                        </tr>
                        <tr className="border-b bg-card">
                          <td className="p-3"><span className="font-medium">Build (minutes)</span></td>
                          <td className="p-3 text-right">{formatUsageValue(deploymentWithBreakdown.usage.build_minutes)}</td>
                          <td className="p-3 text-right">
                            {deploymentWithBreakdown.usage.costs ? (
                              <span className="font-semibold">{deploymentWithBreakdown.usage.costs.build_cost.toFixed(2)}</span>
                            ) : (<span className="text-muted-foreground">-</span>)}
                          </td>
                        </tr>
                        <tr className="border-b last:border-b-0 bg-card">
                          <td className="p-3"><span className="font-medium">Endpoints (hrs)</span></td>
                          <td className="p-3 text-right">{formatUsageValue(deploymentWithBreakdown.usage.public_endpoint_hours)}</td>
                          <td className="p-3 text-right">
                            {deploymentWithBreakdown.usage.costs ? (
                              <span className="font-semibold">{deploymentWithBreakdown.usage.costs.endpoint_cost.toFixed(2)}</span>
                            ) : (<span className="text-muted-foreground">-</span>)}
                          </td>
                        </tr>
                      </tbody>
                      {deploymentWithBreakdown.usage.costs && (
                        <tfoot>
                          <tr className="bg-muted border-t-2">
                            ...
                            <td className="p-3"></td>
                            <td className="p-3 text-right font-semibold">{deploymentWithBreakdown.usage.costs.total_cost.toFixed(2)}</td>
                          </tr>
                        </tfoot>
                      )}
                    </table>
              ...
              {deploymentWithBreakdown.cost_breakdown && (
                <div className="space-y-3">
                  ...
                  deploymentWithBreakdown.cost_breakdown.service_breakdown.length > 0 ? (
                        ...
                        <table className="w-full text-sm">
                          <thead>
                            <tr className="border-b bg-muted">
                              ...
                              <th className="text-right p-3 font-semibold">CPU Usage<span className="text-muted-foreground font-normal text-xs ml-1">(core-hrs)</span></th>
                              <th className="text-right p-3 font-semibold">Memory Usage<span className="text-muted-foreground font-normal text-xs ml-1">(GB-hrs)</span></th>
                              <th className="text-right p-3 font-semibold">Cost<span className="text-muted-foreground font-normal text-xs ml-1">($)</span></th>
                            </tr>
                          </thead>
                          <tbody>
                            {deploymentWithBreakdown.cost_breakdown.service_breakdown.map((service, idx) => (
                              <tr key={service.service_name} className={cn("border-b last:border-b-0", idx % 2 === 0 && "bg-card")}>
                                <td className="p-3">
                                  <div className="flex flex-col">
                                    <span className="font-medium">{service.service_name}</span>
                                    <span className="text-muted-foreground text-xs mt-0.5">{service.percentage_of_total.toFixed(1)}% of deployment cost</span>
                                  </div>
                                </td>
                                <td className="p-3 text-right">{service.cpu_core_hours != null ? <span>{service.cpu_core_hours.toFixed(2)}</span> : <span className="text-muted-foreground">-</span>}</td>
                                <td className="p-3 text-right">{service.memory_gb_hours != null ? <span>{service.memory_gb_hours.toFixed(2)}</span> : <span className="text-muted-foreground">-</span>}</td>
                                <td className="p-3 text-right"><span className="font-semibold">{service.total_compute_cost.toFixed(4)}</span></td>
                              </tr>
                            ))}
                          </tbody>
                          <tfoot>
                            <tr className="bg-muted border-t-2">
                              ...
                              <td className="p-3 text-right font-semibold">{deploymentWithBreakdown.cost_breakdown.service_breakdown.reduce((sum, service) => sum + service.total_compute_cost, 0).toFixed(4)}</td>
                            </tr>
                          </tfoot>
                        </table>
                      </div>
                    ...
                    <p className="text-muted-foreground py-4 text-sm italic text-center">No service breakdown available</p>
                  )}
                </div>
              )}
                    ...
                    <div className="bg-muted rounded-lg border overflow-hidden">
                      <div className="overflow-x-auto">
                        <table className="w-full text-sm">
                          ...
                          <tbody>
                            {deploymentWithBreakdown.cost_breakdown.volume_breakdown.map((volume, idx) => (
                              <tr key={volume.volume_name} className={cn("border-b last:border-b-0", idx % 2 === 0 && "bg-muted/10")}>
                                <td className="p-3"><span className="font-medium">{volume.volume_name}</span></td>
                                <td className="p-3">
                                  <div className="flex flex-col">
                                    <span>{volume.storage_class === "ebs" ? "Standard" : "Shared"}</span>
                                    <span className="text-muted-foreground text-xs mt-0.5">{volume.percentage_of_total.toFixed(1)}% of deployment cost</span>
                                  </div>
                                </td>
                                <td className="p-3 text-right"><span className="font-semibold">{volume.storage_cost.toFixed(4)}</span></td>
                              </tr>
                            ))}
                          </tbody>
                          <tfoot>
                            <tr className="bg-muted border-t-2">
                              ...
                              <td className="p-3 text-right font-semibold">{deploymentWithBreakdown.cost_breakdown.volume_breakdown.reduce((sum, volume) => sum + volume.storage_cost, 0).toFixed(4)}</td>
                            </tr>
                          </tfoot>
                        </table>
                      </div>
                  ...
                  <div className="bg-card rounded-lg border p-3">
                    <div className="flex items-center justify-between text-sm">
                      ...
                      <span className="font-semibold">
                        {(deploymentWithBreakdown.cost_breakdown.service_breakdown.reduce((sum, service) => sum + service.total_compute_cost, 0) +
                          deploymentWithBreakdown.cost_breakdown.volume_breakdown.reduce((sum, volume) => sum + volume.storage_cost, 0)).toFixed(4)}
                      </span>
                    </div>
                    <div className="flex items-center justify-between text-xs text-muted-foreground mt-1">
                      <span>Meter Total ($):</span>
                      <span className="font-semibold text-foreground">{deploymentWithBreakdown.cost_breakdown.meter_breakdown.total_cost.toFixed(4)}</span>
                    </div>
                  </div>
                )}
            </div>
            ...
            {activeDeployments.length > 0 && (
              <>
                <div className="flex items-center gap-2 pt-2 pb-1">
                  <span className="text-sm font-semibold text-green-600 dark:text-green-500">Active</span>
                  <span className="text-muted-foreground bg-muted rounded px-1.5 py-0.5 text-xs">
                    ({activeDeployments.length})
                  </span>
                </div>
                {activeDeployments.map((deployment) => renderDeploymentAccordionItem(deployment))}
              </>
            )}
