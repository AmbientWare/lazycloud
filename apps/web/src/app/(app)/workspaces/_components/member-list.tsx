...
import { getCurrentUserInternalId } from "@/actions/users";
import type {
  WorkspaceMember,
  WorkspaceRole,
  Workspace,
...
import { MembersSkeleton } from "./workspaces-skeletons";
  ...
  useEffect(() => {
    ...
    if (
      ...
      const checkPendingTransfer = async () => {
        try {
          const pending = await getPendingOwnershipTransfer(workspaceId);
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button variant="outline" size="sm" className="relative h-9 shrink-0">
                  ...
                  {(pendingInvitationsCount > 0 || hasPendingOwnershipTransfer) && (
                    <span className="absolute -right-1 -top-1 flex h-5 w-5 items-center justify-center rounded-full bg-blue-500 text-[10px] font-bold text-white">
                      {pendingInvitationsCount + (hasPendingOwnershipTransfer ? 1 : 0)}
                    </span>
                  )}
                </Button>
              </DropdownMenuTrigger>
                    ...
                    <>
                      ...
                      >
                        ...
                        {hasPendingOwnershipTransfer && (
                          <Badge
                            variant="outline"
                            className="ml-auto text-xs bg-purple-500/10 text-purple-500 border-purple-500/30"
                          >
                            Pending
                          </Badge>
                        )}
        ...
        <StyledCardContent>
          <div className="space-y-4">
            <div className="flex flex-col gap-3 sm:flex-row">
              ...
              <Select value={roleFilter} onValueChange={setRoleFilter}>
                ...
                <SelectContent>
                  ...
                  <SelectItem value={WorkspaceRoles.OWNER} className="cursor-pointer">Owner</SelectItem>
                  <SelectItem value={WorkspaceRoles.ADMIN} className="cursor-pointer">Admin</SelectItem>
                  <SelectItem value={WorkspaceRoles.MEMBER} className="cursor-pointer">Member</SelectItem>
                </SelectContent>
              </Select>
            ...
            <div className="space-y-2">
              {isLoading && members.length === 0 ? null : sortedMembers.length === 0 ? (
...
export function MemberListWrapper({
  ...
  const [currentUserRole, setCurrentUserRole] = useState<WorkspaceRole | undefined>();
