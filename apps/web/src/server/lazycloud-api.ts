...
class LazyCloudAPIClass {
  ...
  private async fetchWithApiUrl<T>(
    ...
    if (token) {
      ...
      (authOptions.accessToken !== undefined || authOptions.useAdmin)
    ) {
  ...
  async getWorkspaces(
    ...
    return await this.get<Workspace[]>(`/workspaces${queryString}`, {
      accessToken,
    })
  }
  ...
  async getCurrentUser(
    accessToken: string,
  ): Promise<{ id: string; workos_id: string }> {
    return await this.get<{ id: string; workos_id: string }>(`/users/current`, {
      accessToken,
    })
  }

  async getUserFeatures(accessToken: string): Promise<UserFeaturesResponse> {
    return await this.get<UserFeaturesResponse>(`/users/features`, {
      accessToken,
    })
  }
  ...
  async getBillingCycle(accessToken: string): Promise<BillingCycleResponse> {
    return await this.get<BillingCycleResponse>('/billing/cycle', {
      accessToken,
    })
  }
  ...
  async getPendingInvitations(accessToken: string): Promise<
    {
      workspace_id: string
