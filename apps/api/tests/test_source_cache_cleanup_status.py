...
def test_source_cache_cleanup_status_is_admin_only_and_bounded(
    ...
    with ExitStack() as client_stack:
        ...
        admin_token, _admin = administrator_credential(
            isolated_services.context, "cleanup-status-admin"
        )
...
def test_source_cache_cleanup_status_returns_not_found_for_unknown_workspace(
    ...
    token, _ = administrator_credential(services.context, "cleanup-status-admin")
    response = client.get(
