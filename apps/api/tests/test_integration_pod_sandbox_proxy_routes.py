...
def test_generated_pod_url_preserves_request_and_selects_port_ready_container(
    ...
    with ExitStack() as client_stack:
        ...
        assert connections.total[stub.id] == 0
...
def test_pod_proxy_records_demand_before_waiting_for_scale_from_zero(
    ...
    with ExitStack() as client_stack:
        ...
        assert response.status_code == 209
        assert connections.total[stub.id] == 0
        assert connections.active[container.id] == 0
    ...
    with ExitStack() as client_stack:
        ...
        try:
            ...
            with client.websocket_connect(
                ...
                assert connections.total[stub.id] == 1
                assert connections.active[container.id] == 1
            ...
            assert not backend_thread.is_alive(), "websocket backend did not stop"
...
def test_pinned_sandbox_routes_never_wait_or_fall_through_to_a_sibling(
    ...
    with ExitStack() as client_stack:
        ...
        assert (connections.total, connections.active) == connection_counts
        ...
        stopped_counts = (dict(connections.total), dict(connections.active))
        started = time.monotonic()
        unavailable = client.get(f"/sandbox/id/{first.id}/8080", headers=headers)
        elapsed = time.monotonic() - started
        assert (connections.total, connections.active) == stopped_counts
...
def test_sandbox_proxy_supports_id_deployment_and_public_path_forms(
    ...
    with ExitStack() as client_stack:
        ...
        latest_response = client.get(
            f"/sandbox/{deployment.name}/latest/7000/health", headers=headers
        )
