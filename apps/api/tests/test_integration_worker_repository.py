...
def test_cache_origin_broker_denies_other_workers_container_and_image(
    ...
    with ExitStack() as client_stack:
        ...
        repository = isolated_services.worker_repository_service
        ...
        credentials = GetCacheOriginCredentialsResponse.model_validate_json(
            allowed.content
        ).credentials
...
async def test_worker_stream_rechecks_cache_after_dequeue_and_requeues_on_drain(
    ...
    service = async_services.worker_repository_service
    source_cache = _SecondCheckUnavailableSourceCache(async_services.context)
...
@pytest.mark.anyio
async def test_worker_event_stream_excludes_foreign_and_unassigned_stop_events(
    async_services: ApiServices,
) -> None:
    service = async_services.worker_repository_service
    events = [
        EventBusEvent(
            type=EventBusEventType.StopContainer,
            args={"container_id": "foreign", "worker_id": "worker-2", "reason": "user"},
        ),
        EventBusEvent(
            type=EventBusEventType.StopContainer,
            args={"container_id": "unassigned", "reason": "user"},
        ),
        EventBusEvent(
            type=EventBusEventType.StopContainer,
            args={"container_id": "owned", "worker_id": "worker-1", "reason": "user"},
        ),
    ]
    event_ids = [service.events.send(event).event_id for event in events]
    stream = service.stream_worker_events(
        async_services.require_async_io(),
        StreamWorkerEventsRequest(worker_id="worker-1", event_ids=event_ids, max_events=1),
    )

    async def collect() -> list[WorkerStreamEvent]:
        return [event async for event in stream]

    delivered = await asyncio.wait_for(collect(), timeout=1)
