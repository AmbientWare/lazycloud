...
from lazycloud_api.tests.e2e.conftest import (
    E2E_WORKSPACE_NAME,
    cleanup_workspace,
    get_output,
    requires_api,
...
class TestCLIWorkspaceCreateDestroy:
    ...
    def test_workspace_create_duplicate_fails(self):
        ...
        create_result = run_cli("workspace", "create", test_ws_name)
        if create_result.returncode != 0:
            pytest.skip("Could not create initial workspace - may be at limit")

        try:
            duplicate_result = run_cli("workspace", "create", test_ws_name)
...
class TestCLIWorkspaceActivation:
    ...
    def test_workspace_activate_nonexistent(self):
        ...
        result = run_cli("workspace", "activate", "nonexistent-workspace-xyz")
        assert result.returncode != 0
    ...
    def test_workspace_activate_flow(self):
        ...
        create_result = run_cli("workspace", "create", self.E2E_ACTIVATE_WORKSPACE)
        if create_result.returncode != 0:
            pytest.skip("Cannot create workspace - may be at limit")

        try:
            activate_result = run_cli(
                "workspace", "activate", self.E2E_ACTIVATE_WORKSPACE
            )
...
class TestCLIWorkspaceDestroy:
    ...
    def test_workspace_destroy_nonexistent(self):
        ...
        result = run_cli("workspace", "destroy", "nonexistent-workspace-xyz", "--force")
    ...
    def test_workspace_destroy_requires_force_or_confirmation(self):
        ...
        create_result = run_cli("workspace", "create", test_ws)
        if create_result.returncode != 0:
            pytest.skip("Cannot create workspace - may be at limit")

        try:
            # Without --force, should fail due to non-interactive mode
            destroy_result = run_cli("workspace", "destroy", test_ws)
