...
`cloudflared.yml` lists every public route: the tunnel is locally managed, so
nothing outside this file adds or removes one. `README.md` covers operating and
rotating it.
...
- Refusing something at the edge takes it off the internet; it never replaces
  the origin's own authorization. Do not relax an origin check because a prefix
  is blocked here.
