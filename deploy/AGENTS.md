...
- Migration waves finish before new application pods start, while previous
  replicas continue serving. New durable ownership fields belong in relational
  columns whose defaults permit existing writers. Keep strict JSON projections
  readable during the rollout and prove that an earlier writer cannot erase
  ownership recorded by the new repository.
...
- One complete manifest selects platform images, the worker image, agent binary,
  ...
  file and admit workers only when the selected artifacts match registration.
  Managed hosts use the existing replacement controller; supervised joined agents
  update after their workloads drain. Release observation needs no database tables.
