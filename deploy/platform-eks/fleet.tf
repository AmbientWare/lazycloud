...
# nodes need egress to pull images and reach Pangolin, they accept nothing
...
# private network, which is a session the node itself establishes outbound.
resource "aws_vpc_security_group_egress_rule" "fleet_node" {
  security_group_id = aws_security_group.fleet_node.id
  description       = "Image pulls, Pangolin, and the control plane."
  cidr_ipv4         = "0.0.0.0/0"
  ip_protocol       = "-1"
}
