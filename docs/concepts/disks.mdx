...
one machine. A machine failure can lose up to the last two minutes of writes.

A start does not wait for the disk to download, however large it is. Data
arrives the first time something reads it, starting with what the last
session used, and the rest follows in the background. The first read of a file
nobody has touched in a while can take a moment longer. A restart within 30
minutes on a machine that still holds the disk reads nothing from storage. If
part of a disk cannot be fetched, the container stops and says so.
