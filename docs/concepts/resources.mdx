...
constructor, and nothing outside the code overrides them.
...
set it. Files that must outlive it belong in a [volume](/concepts/volumes).
...
`GpuType` names the models: `T4`, `A10G`, `L4`, `L40S`, `A100_40`, `A100_80`,
`H100`, and `H200`. A list sets a preference order, best first.
`gpu=[GpuType.H100, GpuType.A100_80, GpuType.L4]` takes the first model with
free capacity, and `GpuType.Any`, valid only at the end of a list, takes
whatever is free. The two A100 sizes are not interchangeable, so each has its
own member and plain `GpuType.A100` is rejected. `gpu_count` sets GPUs per
container. GPU access and prices depend on your [plan](/platform/plans).
...
its retry policy, and LazyCloud replaces a preempted endpoint container.
Functions that are safe to repeat, and long jobs that checkpoint to a volume,
lose little to a preemption.
