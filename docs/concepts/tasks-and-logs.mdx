...
```bash
uv run lazycloud task list --app reports
uv run lazycloud task show <task-id>
uv run lazycloud task result <task-id>
uv run lazycloud logs --task-id <task-id> --follow
```

`logs` also reads by `--deployment` or `--container-id`. Anything your code
prints is captured; print with `flush=True` or run Python unbuffered so lines
appear promptly. See the [task and logs reference](/cli/tasks-and-logs) for
every flag. The dashboard's Tasks page shows the same history.
...
Failed tasks keep their error and full logs. Logs are kept for your plan's
retention window; see [Plans and limits](/platform/plans).
