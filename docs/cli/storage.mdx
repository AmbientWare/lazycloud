...
[secret dependencies](/concepts/secrets) in Python. LazyCloud creates a mounted
volume on first use. Use these commands for one-off storage work and inspection.
...
The example below creates a volume before any workload uses it. Skip
`volume create` if a running workload already created its declared volume.
...
Use `uv run lazycloud artifact delete <artifact-id>` to remove a saved file.
Lists return a `next` cursor. Pass `--cursor <cursor>` for the next page. See
