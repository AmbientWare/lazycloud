...
## lazycloud devbox login

```bash
uv run lazycloud devbox login NAME HARNESS [--app <app>] [--workspace <workspace>]
```

Runs the installed agent's native browser login inside the devbox. `HARNESS` is
`codex`, `claude-code`, `opencode`, or `pi`. The local CLI opens your browser and
