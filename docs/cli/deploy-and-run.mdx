...
uv run lazycloud ssh-config [NAME...] [--app <app>]
```
...
`ssh`, one per name. With no name it writes one for every devbox and SSH pod
in the workspace and removes the workspace's entries for ones that stopped or
were deleted. `--app` picks the app when several apps deploy the same name. The generated
