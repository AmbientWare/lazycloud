...
```bash
uv run lazycloud devbox list [--app <app>] [--limit <n>] [--cursor <cursor>]
uv run lazycloud devbox NAME status [--app <app>]
uv run lazycloud devbox NAME ssh [--app <app>] [-- <ssh options> <command>]
uv run lazycloud devbox NAME login HARNESS [--app <app>]
```

`list` returns a page of deployed devboxes. `status` shows one box's live state
and resources. Both support global `--json` output and leave sleeping boxes
asleep. `ssh` opens a session. All four commands accept `--workspace <workspace>`.
See [devboxes](/concepts/dev-machines#find-and-inspect-boxes) for listing and selection.

`login` runs the installed agent's native browser login inside the devbox. `HARNESS` is
`codex`, `claude`, `opencode`, or `pi`. The local CLI opens your browser and
temporarily forwards the callback over SSH. It requires an interactive terminal
and does not support `--json`. See [coding agents](/concepts/dev-machines#coding-agents)
for installation, provider selection, and credential persistence.
