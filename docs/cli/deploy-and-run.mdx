...
```bash
uv run lazycloud ssh POD [--app <app>]
uv run lazycloud ssh-config [POD...] [--app <app>]
```

`ssh` opens a session on a deployed pod that sets `ssh=True`. `ssh-config`
...
named. `--app` is needed only when several
apps deploy a pod with the same name. The generated config calls
