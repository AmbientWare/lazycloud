...
Prints a link and a code. Approve the code in the browser to sign the CLI in
as you. See [Sign-in and workspaces](/platform/auth).

CI needs no login step. Set `LAZYCLOUD_TOKEN` from your CI secret store and
`LAZYCLOUD_WORKSPACE` to the target workspace. To save an existing environment
token in a local profile:
...
Create and delete tokens in the dashboard under Settings, then Tokens. These
