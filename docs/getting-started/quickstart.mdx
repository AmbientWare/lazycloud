...
with no upload and no charge. `.remote()` runs the same function in a container
with 1 CPU and 256 MiB, streams its logs back, and returns `hello LazyCloud`.
The first remote call takes longer while the image builds.

The CLI makes the remote call without a `__main__` block:
