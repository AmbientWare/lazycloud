...
The downloaded project's `app.py` contains both functions and their resources.
...
`app.py`. Training writes a checkpoint and metrics;
...
If training fails, use `uv run lazycloud task list --app yolo_training` and inspect
