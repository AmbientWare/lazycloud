...
Source is `examples/parallel_parquet/app.py`. PyArrow installs in the remote
...
The last two variables hold secret names, not credentials. Keep the configuration
exported in the terminal where you run the batch.
...
If you use a connected AWS account instead, you can omit both secret-name
variables and use the instance role. Set `pool="aws"` on all four function
decorators in the source so local remote calls select that capacity. Grant the
role the same bucket permissions. See [cloud buckets](/concepts/volumes#cloud-buckets).
