...
class FlyMetricsCollector:
    ...
    async def _fetch_metrics(
        self, start_time: datetime, end_time: datetime, usage_uuid: str
    ):
        ...
        instance_params = {
            "query": f'sum by (memory_mb, cpu_count, cpu_kind, app) (count by (memory_mb, cpu_count, cpu_kind, app, instance_id) (fly_instance_info{{app=~"lc-{usage_uuid}-.*"}}))',
    ...
    def _combine_metrics(self, instance_data: dict, data_out_data: dict):
        ...
        for result in instance_data.get("data", {}).get("result", []):
            ...
            # Add to the app's total minutes
            combined[app]["total_minutes"] += total_minutes
    ...
    async def collect_metrics(self, start_time: datetime, end_time: datetime):
        ...
        try:
            ...
            for usage_entry in usage_entries:
                ...
                instance_data, data_out_data = await self._fetch_metrics(
                    start_time, end_time, usage_uuid
                )

                combined_metrics = self._combine_metrics(instance_data, data_out_data)
...
async def main():
    async with FlyMetricsCollector() as collector:
        # Get today's metrics
        print("\n=== Today's Metrics ===")
        start_time = datetime.now().replace(hour=0, minute=0, second=0, microsecond=0)
        end_time = datetime.now()
        today_metrics = await collector.collect_metrics(start_time, end_time)
