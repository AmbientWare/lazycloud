...
def _sync_image_build_row(row: ImageBuildTable, build: ImageBuildRecord) -> None:
    ...
    row.artifact_path_value = normalize_runtime_path(build.artifact_path)
    row.runtime_path_digest = runtime_path_digest(row.artifact_path_value)
    row.manifest_path_value = normalize_runtime_path(build.manifest_path)
    row.manifest_path_digest = runtime_path_digest(row.manifest_path_value)
    row.dockerfile_path_value = normalize_runtime_path(
        build.cache_metadata.get("dockerfile_path", "")
    )
    row.dockerfile_path_digest = runtime_path_digest(row.dockerfile_path_value)
    row.cache_manifest_path_value = normalize_runtime_path(
        build.cache_metadata.get("manifest_path", "")
    )
    row.cache_manifest_path_digest = runtime_path_digest(row.cache_manifest_path_value)
