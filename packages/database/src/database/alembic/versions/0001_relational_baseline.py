...
CREATE TABLE autoscaler_states (
	...
	owner_lock_key TEXT NOT NULL,
	cooldown_until TIMESTAMP WITH TIME ZONE,
	failed_container_count BIGINT NOT NULL,
	pending_count BIGINT NOT NULL,
	guardrails JSONB NOT NULL,
	last_actions JSONB NOT NULL,
	created_at TIMESTAMP WITH TIME ZONE DEFAULT now() NOT NULL,
	updated_at TIMESTAMP WITH TIME ZONE DEFAULT now() NOT NULL,
	PRIMARY KEY (workspace_id, target_kind, target_id),
	CONSTRAINT ck_autoscaler_states_counts CHECK (current_count >= 0 AND desired_count >= 0 AND pending_count >= 0 AND failed_container_count >= 0),
	CONSTRAINT ck_autoscaler_states_target_kind CHECK (target_kind IN ('function', 'endpoint', 'pod')),
	FOREIGN KEY(workspace_id) REFERENCES workspaces (id) ON DELETE CASCADE
)
""")
    op.execute("""
