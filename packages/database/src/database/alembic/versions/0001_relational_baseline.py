...
CREATE TABLE compute_units (
	warm_handoff_from JSONB DEFAULT '[]' NOT NULL,
	...
	worker_runtimes JSONB NOT NULL,
	...
	CONSTRAINT ck_compute_units_internal_provider_identity CHECK (visibility <> 'internal' OR (provider_ref <> '' AND region <> '' AND offer_id <> '' AND capability_key <> '' AND capacity_mode = 'pooled' AND capacity_owner_kind = 'pooled_provider' AND capacity_owner_id = id AND (provider_connection_id IS NOT NULL OR COALESCE(CAST(payload->>'platform_fleet' AS BOOLEAN), false)))),
...
CREATE TABLE compute_capacity_operations (
	...
	CONSTRAINT ck_compute_capacity_operations_desired_unit CHECK (desired_unit > 0),
