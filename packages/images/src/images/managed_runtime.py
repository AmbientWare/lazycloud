...
def load_managed_runtime_catalog(
    ...
    root = root.resolve()
    catalog_path = root / MANAGED_RUNTIME_CATALOG_FILE
    ...
    if available_versions != expected_versions:
        ...
        unexpected = sorted(available_versions - expected_versions)
        details = [
            *(f"missing {version}" for version in missing),
            *(f"unexpected {version}" for version in unexpected),
        ]
    ...
    expected_source_digest = managed_package_source_digest()
    if len(expected_source_digest) != 64 or manifest.source_digest != expected_source_digest:
        raise RuntimeError(
            "managed runtime source digest mismatch: "
            f"expected {expected_source_digest or 'unavailable'}, got {manifest.source_digest}"
        )
    launcher_path = root / MANAGED_RUNTIME_LAUNCHER_FILE
