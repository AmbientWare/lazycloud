...
def test_compute_state_repository_tracks_pools_agents_slots_and_ttls(
    ...
    assert 0 <= redis.ttl(redis.key("compute:join-tokens:join-hash")) <= 1
    ...
    assert redis.get(repo.keys.agent_route_revision("ws-1", OWNER_ID, "machine-1")) == "1"
    ...
    redis.set(orphan_revision, "1")
    redis.set(peer_revision, "1")

    assert repo.delete_unit_state("ws-1", ORPHAN_OWNER_ID)
    assert redis.get(orphan_revision) is None
...
def test_compute_state_repository_deletes_exact_workspace_residue(
    real_redis_actors: RealRedisActors,
) -> None:
    redis = real_redis_actors.client()
    repo = RedisComputeStateRepository(redis)
    ...
    assert repo.redis.scan(redis.key("compute:workspaces:ws-delete:*")) == []

    assert repo.get_join_token_state("join-peer") is not None
    assert repo.get_agent_token_state("agent-peer") is not None
    assert repo.redis.scan(redis.key("compute:workspaces:ws-peer:*")) != []
