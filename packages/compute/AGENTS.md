...
| Compute row | `preparing` -> `stopping` -> `stopped` -> `resuming` -> `active` | preparation completes once the worker is ready (warm: release running and waiting; plain: no worker); the pass, or a recheck every few seconds once the provider accepts the stop, observes it; the pass starts the instance; the stream after the one that authorizes the resume, so that answer waits on no provider call |
