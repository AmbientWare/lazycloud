...
def test_disks_stay_within_the_plans_declared_size_per_workspace(
    isolated_services: ApiServices,
) -> None:
...
def _disk_names(services: ApiServices, workspace_id: str) -> list[str]:
    with services.context.database.session() as session:
        return sorted(
            session.scalars(select(DiskTable.name).where(DiskTable.workspace_id == workspace_id))
        )
