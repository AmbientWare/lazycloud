...
def publish_metered_rate_history(session: Session) -> tuple[MeteredRatePublication, ...]:
    return tuple(
        publish_metered_rate_change(session, card) for card in PUBLISHED_METERED_RATE_HISTORY
    )


def publish_metered_rate_change(
    session: Session, card: MeteredRateChange
) -> MeteredRatePublication:
