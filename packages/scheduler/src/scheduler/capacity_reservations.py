...
@dataclass(slots=True)
class StaticWorkerPoolCapacityController:
    pool: Pool
    ...
    def operational_health(
        ...
        *,
        ...
        return capacity_pool_operational_health(
            self.capacity_owner_id,
            self.workers.list_workers(),
            state=self.sizing_states.get_pool_sizing_state(self.capacity_owner_id),
            now=now,
        )
    ...
    def reconcile_sizing(
        ...
        *,
        ...
        registered_units = headroom.available_workers + headroom.unclaimed_pending_workers
        state = _sizing_state(self.pool, self.sizing_states, registered_units)
        describe_operation_id = state.operation_id or str(uuid4())
        try:
            authoritative = self.scaler.describe_worker_pool(
                self.pool,
                reservation_id=describe_operation_id,
                operation_id=describe_operation_id,
            )
        except Exception:
            _save_sizing_state(
                self.sizing_states,
                sizing_failure_state(state, self.pool, now=now).model_copy(
                    update={"terminal_reason": "worker-pool state read failed"}
                ),
            )
            return WorkerPoolSizingPlan(
                ...
                current_units=registered_units,
                target_units=state.target_units or registered_units,
                headroom=headroom,
                initial_target_reached=state.initial_target_reached,
                reason="worker-pool state read failed and entered durable backoff",
            )
        if authoritative.outcome in {
            WorkerPoolReplicaScaleOutcome.TemporarilyUnavailable,
            WorkerPoolReplicaScaleOutcome.Unsupported,
        }:
            _save_sizing_state(
                self.sizing_states,
                sizing_failure_state(state, self.pool, now=now).model_copy(
                    update={"terminal_reason": authoritative.reason}
                ),
            )
            return WorkerPoolSizingPlan(
                action=WorkerPoolSizingAction.Wait,
                ...
                headroom=headroom,
                initial_target_reached=state.initial_target_reached,
                reason=authoritative.reason or "worker-pool state is temporarily unavailable",
            )
        if authoritative.outcome is WorkerPoolReplicaScaleOutcome.AtLimit:
            _save_sizing_state(
                self.sizing_states,
                state.model_copy(
                    update={
                        "operation_id": "",
                        "operation_started_at": None,
                        "target_units": authoritative.desired_replicas,
                        "terminal_reason": authoritative.reason or "worker pool is at limit",
                    }
                ),
            )
            return WorkerPoolSizingPlan(
                ...
                capacity_owner_id=self.capacity_owner_id,
                pool_name=self.pool_name,
                current_units=max(registered_units, authoritative.desired_replicas),
                target_units=authoritative.desired_replicas,
                headroom=headroom,
                initial_target_reached=state.initial_target_reached,
                reason=authoritative.reason or "worker pool is at limit",
            )
        plan = plan_worker_pool_sizing(
            self.pool,
            headroom=headroom,
            registered_units=registered_units,
            authoritative_units=authoritative.desired_replicas,
            state=state,
            now=now,
        )
        state = _record_sizing_observation(self.sizing_states, state, plan)
        if state.operation_id and registered_units < state.target_units:
            if state.retry_after_at is not None and state.retry_after_at > now:
                return plan
            if authoritative.desired_replicas >= state.target_units:
                return plan
            return self._scale_for_sizing(state, plan, now=now)
        if plan.action is not WorkerPoolSizingAction.ScaleUp:
            return plan
        operation_id = str(uuid4())
        state = _save_sizing_state(
        ...
        *,
        ...
        try:
            ...
            _save_sizing_state(
                self.sizing_states,
                sizing_failure_state(state, self.pool, now=now).model_copy(
                    update={"terminal_reason": "worker-pool scale-up failed"}
                ),
            )
            return plan.model_copy(
                update={
                    "action": WorkerPoolSizingAction.Wait,
                    "reason": "worker-pool scale-up failed and entered durable backoff",
                }
            )
        if result.outcome in {
            ...
            return plan.model_copy(
                update={
                    "action": WorkerPoolSizingAction.Wait,
                    "reason": result.reason or "worker-pool scale-up is temporarily unavailable",
                }
            )
        if result.outcome is WorkerPoolReplicaScaleOutcome.AtLimit:
            _save_sizing_state(
                self.sizing_states,
                state.model_copy(
                    update={
                        ...
                        "terminal_reason": result.reason or "worker pool is at limit",
                    }
                ),
            )
            return plan.model_copy(
                update={
                    "action": WorkerPoolSizingAction.None_,
                    "target_units": result.desired_replicas,
                    "reason": result.reason or "worker pool is at limit",
                }
            )
        _save_sizing_state(
            self.sizing_states,
            state.model_copy(
                update={
                    ...
                    "last_scale_up_at": (
                        now
                        if result.outcome is WorkerPoolReplicaScaleOutcome.Requested
                        else state.last_scale_up_at
                    ),
                    "retry_after_at": None,
                    "consecutive_failures": 0,
        ...
        return plan.model_copy(update={"target_units": result.desired_replicas})
    ...
    def plan_acquisition(
        ...
        *,
        ...
        _ = now
        current = self.scaler.describe_worker_pool(
            self.pool,
            reservation_id=reservation.id,
            operation_id=reservation.operation_id,
        )
        if current.outcome in {
            ...
            return _replica_acquisition_result(reservation, current)
        registered = sum(
            1
            for worker in self.workers.list_workers()
            if worker.capacity_owner_id == self.capacity_owner_id
            and worker.status in {SchedulerWorkerStatus.Pending, SchedulerWorkerStatus.Available}
        )
        claimed_pending_units = sum(
            candidate.id != reservation.id
            and candidate.accepting_allocations
            and candidate.source is CapacityReservationSource.PlacementMiss
            and not candidate.target_worker_id
            for candidate in owner_reservations
        )
        if current.desired_replicas - registered - claimed_pending_units > 0:
            return CapacityAcquisitionResult(
                status=CapacityAcquisitionStatus.ExistingPending,
                capacity_owner_id=self.capacity_owner_id,
                reservation_id=reservation.id,
                operation_id=reservation.operation_id,
                desired_unit=current.desired_replicas,
                reason="authoritative worker replica is awaiting registration",
            )
        if current.desired_replicas >= self.pool.max_workers:
            return CapacityAcquisitionResult(
                status=CapacityAcquisitionStatus.AtLimit,
                capacity_owner_id=self.capacity_owner_id,
                reservation_id=reservation.id,
                operation_id=reservation.operation_id,
                desired_unit=current.desired_replicas,
                reason="durable worker pool maximum is exhausted",
            )
        return CapacityAcquisitionResult(
            ...
            reservation_id=reservation.id,
            operation_id=reservation.operation_id,
            desired_unit=current.desired_replicas + 1,
            reason="planned one bounded worker replica",
        )
    ...
    def ensure_acquisition(
        ...
        *,
        ...
        _ = now
        if reservation.desired_unit <= 0:
            raise ValueError("static worker acquisition requires a persisted desired replica")
        return _replica_acquisition_result(
            reservation,
            self.scaler.scale_worker_pool(
                self.pool,
                reservation.desired_unit,
                reservation_id=reservation.id,
                operation_id=reservation.operation_id,
            ),
        )
    ...
    def reconcile(
        ...
        *,
        ...
        if (
            current.outcome is WorkerPoolReplicaScaleOutcome.ExistingPending
            and reservation.desired_unit > 0
            and current.desired_replicas < reservation.desired_unit
        ):
            current = self.scaler.scale_worker_pool(
                self.pool,
                reservation.desired_unit,
                reservation_id=reservation.id,
                operation_id=reservation.operation_id,
    ...
    def plan_release(
        ...
        *,
        ...
        return CapacityAcquisitionResult(
            ...
            reservation_id=reservation.id,
            operation_id=reservation.operation_id,
            desired_unit=max(current.desired_replicas - 1, self.pool.min_workers),
            reason="planned removal of the reservation-owned worker replica",
        )
        ...
        *,
        ...
        if not reservation.acquisition_created:
            return CapacityAcquisitionResult(
                ...
                operation_id=reservation.operation_id,
                desired_unit=reservation.desired_unit,
                reason="reservation reused existing capacity and owns no replica to release",
            )
        if not reservation.release_requested or reservation.release_target_unit is None:
            raise ValueError("static capacity release requires a persisted release target")
        released = self.scaler.scale_worker_pool(
            self.pool,
            reservation.release_target_unit,
            reservation_id=reservation.id,
            operation_id=reservation.operation_id,
        )
        return _replica_acquisition_result(reservation, released)
...
class RedisCapacityReservationRepository:
    ...
    def prepare_release(
        ...
        *,
        ...
        rebased: list[CapacityProvisioningReservation] = []
        if rebase_fungible_units:
            for candidate in self.list_for_owner(current.capacity_owner_id):
                if (
                    candidate.id == current.id
                    or not candidate.accepting_allocations
                    or not candidate.acquisition_created
                    or candidate.desired_unit <= current.desired_unit
                ):
                    continue
                rebased.append(
                    candidate.model_copy(
                        update={
                            "resource_version": candidate.resource_version + 1,
                            "desired_unit": candidate.desired_unit - 1,
                            "updated_at": now,
                        }
                    )
                )
        pipeline = self.redis.pipeline(transaction=True)
        pipeline.set(self.keys.reservation(prepared.id), prepared.model_dump_json())
        for candidate in rebased:
            pipeline.set(self.keys.reservation(candidate.id), candidate.model_dump_json())
        pipeline.execute()
        return prepared
        ...
        *,
        ...
        if not reservation.release_requested:
            ...
            reservation = self.reservations.prepare_release(
                ...
                rebase_fungible_units=(
                    reservation.owner_kind is CapacityOwnerKind.GlobalKubernetesDeployment
                ),
                now=now,
            )
            owner_reservations = tuple(
...
def _replica_acquisition_result(
    reservation: CapacityProvisioningReservation,
    result: WorkerPoolReplicaScaleResult,
) -> CapacityAcquisitionResult:
    if result.capacity_owner_id != reservation.capacity_owner_id:
        raise ValueError("replica scaler returned a different capacity owner")
    if result.pool_name != reservation.pool_name:
        raise ValueError("replica scaler returned a different pool")
    return CapacityAcquisitionResult(
        status=CapacityAcquisitionStatus(result.outcome.value),
        capacity_owner_id=reservation.capacity_owner_id,
        reservation_id=reservation.id,
        operation_id=reservation.operation_id,
        desired_unit=result.desired_replicas,
