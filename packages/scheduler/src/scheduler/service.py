...
class SchedulerDiskSnapshotService(Protocol):
    def reconcile_due(self, *, now: datetime | None = None, limit: int = 100) -> None: ...
...
class SchedulerMaintenanceControls:
    ...
    disk_snapshots: SchedulerDiskSnapshotService | None = None
...
class Scheduler:
    ...
    def run_housekeeping_pass(
        ...
        *,
        ...
        if self.maintenance.disk_snapshots is not None:
            self.maintenance.disk_snapshots.reconcile_due(now=now, limit=container_limit)
