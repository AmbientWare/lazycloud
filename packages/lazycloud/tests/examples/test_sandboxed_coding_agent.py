...
import pytest
from lazycloud._examples.sandboxed_coding_agent.project.sandboxed_coding_agent import (
    app as agent_module,
)
from lazycloud._examples.sandboxed_coding_agent.project.sandboxed_coding_agent.app import (
    MAX_PATCH_BYTES,
    PatchPlan,
    _bounded_output,
...
def test_patch_plan_rejects_duplicates_unapproved_paths_and_binary_content() -> None:
    ...
    with pytest.raises(ValueError, match="not approved"):
        validate_patch_plan({"files": [{"path": "check_calculator.py", "content": "pass"}]})
...
def test_planner_serializes_provider_request_and_validates_structured_response(
    ...
    seed = {
        ...
        "check_calculator.py": "assert add(2, 3) == 5\n",
    }
...
def test_planner_rejects_invalid_provider_json_without_echoing_it(
    ...
    with pytest.raises(RuntimeError) as error:
        plan_patch.local(
            "Fix addition",
            {"calculator.py": "bad", "check_calculator.py": "test"},
        )
...
def test_planner_bounds_direct_seed_inputs_before_calling_the_provider(
    ...
    with pytest.raises(ValueError, match="24000"):
        plan_patch.local(
            "Fix addition",
            {"calculator.py": "x" * 24_001, "check_calculator.py": "test"},
        )
