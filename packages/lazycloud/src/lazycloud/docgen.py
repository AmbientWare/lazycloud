...
def render_sdk_reference(
    ...
    lines = [
        ...
        "  `@app.function`, `@app.endpoint`, and `@app.asgi`.",
        "- Function objects call the remote execution service when invoked directly.",
        "  Use `.local(*args, **kwargs)` for in-process execution.",
        "- Endpoint objects follow the beta-style resource surface: they are invoked",
        "  over HTTP, and `.local(...)` is reserved for explicit in-process execution.",
