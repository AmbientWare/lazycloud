...
Configure provider secrets using https://docs.lazycloud.dev/examples/sandboxed-coding-agent,
then run from the downloaded project directory:
...
def plan_patch(prompt: str, seed_files: dict[str, str]) -> PatchPlan:
    ...
    if set(seed_files) != {"calculator.py", "check_calculator.py"}:
...
def run_agent(
    ...
    try:
        ...
        test_result = instance.run(
            ["python", "-m", "project.check_calculator", "-v"],
            timeout_seconds=60,
            cwd=str(SANDBOX_PROJECT_ROOT.parent),
        )
        return {
